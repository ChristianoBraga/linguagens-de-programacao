...
* Lado direito primeiro, `prox(c)` vale 1 ali. O índice é avaliado depois e vale 2. O índice 2 recebe 1.

* Argumentos da esquerda para a direita, `f(prox(c), prox(c))` é `f(1, 2)`.
