...
O cliente, a função `main`, cria uma pilha, empilha dois valores e os desempilha. Ele nunca menciona `itens` nem `topo`. O construtor `Pilha(int n)` roda quando `new Pilha(8)` cria o objeto e dá aos campos os primeiros valores, e cada método lê e escreve os campos por `this`, ou pelo nome do campo sozinho, que dentro de um método denota o campo de `this`.
...
Uma representação em geral satisfaz uma propriedade que toda operação preserva, o *invariante de representação*. Para a pilha, o contador `topo` fica entre zero e a capacidade do vetor, e os valores abaixo de `topo` são os empilhados e ainda não desempilhados. A visibilidade é o que torna o invariante demonstrável. Como só os métodos escrevem os campos, verificar que cada método preserva a propriedade, supondo que ela vale na entrada, basta para saber que ela vale em todo ponto de todo cliente.

Core C++ torna visível uma consequência de um invariante quebrado. A pilha acima não verifica os seus limites, e um cliente que empilha nove valores em uma pilha de capacidade oito faz `empilha` escrever no índice oito de um vetor de tamanho oito. Em C++ essa escrita é comportamento indefinido. Em Core C++ ela é o resultado `erro`, pela regra `LocIndex` da {secref}[aula-7], e o programa para na operação que quebrou o contrato. Uma pilha que confere `topo` com a capacidade, e reporta o estouro ao cliente por um resultado `bool`, mantém o invariante por conta própria.
...
{exercise "exr-swap-representation"}[] Reescreva a pilha da {secref}[assinatura] sobre uma lista encadeada de objetos `No` em vez de um vetor, mantendo a assinatura. Confira que a função `main` do exemplo roda sem alteração.

{exercise "exr-invariant-check"}[] Mude `empilha` para devolver um `bool`, falso quando a pilha está cheia, e explique por que o invariante então vale para todo cliente, com a regra que de outro modo produziria `erro`.

{exercise "exr-class-table"}[] Para as classes da {secref}[assinatura], escreva a tabela de classes como o verificador a vê, e o contexto Γ sob o qual o corpo de `desempilha` é verificado.
