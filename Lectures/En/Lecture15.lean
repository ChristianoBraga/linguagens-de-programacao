...
ρ₀ = []                    σ₀ = {}
ρ₁ = [n ↦ ℓ0]              σ₁ = {ℓ0 ↦ 2}
ρ₂ = [n ↦ ℓ0, f ↦ ℓ1]      σ₂ = {ℓ0 ↦ 2, ℓ1 ↦ closure(int x)[n ↦ 2]}
ρ₃ = [n ↦ ℓ2, x ↦ ℓ3]      σ₃ = {ℓ0 ↦ 2, ℓ1 ↦ closure(int x)[n ↦ 2], ℓ2 ↦ 2, ℓ3 ↦ 40}
c₀ = std::function<int(int)> f = [=](int x) -> int { return x + n; };

                   𝒟₄                                  𝒟₅                                   𝒟₆
  ρ₀, σ₀ ⊢ int n = 2; ⇒ normal, ρ₁, σ₁    ρ₁, σ₁ ⊢ c₀ ⇒ normal, ρ₂, σ₂    ρ₂, σ₂ ⊢ return f(40); ⇒ ret 42, ρ₂, σ₂
  ─────────────────────────────────────────────────────────────────────────────────────────────────────────────── (Call)
  ρ₀, σ₀ ⊢ main() ⇒ 42, σ₀
...
𝒟₁
  ...
  ρ₂, σ₂ ⊢ f ⇒ₗ ℓ1, σ₂
  ────────────────────────────────────── (Var)
  ρ₂, σ₂ ⊢ f ⇒ closure(int x)[n ↦ 2], σ₂

𝒟₂
  ──────────────────── (LocVar)          ──────────────────── (LocVar)
  ρ₃, σ₃ ⊢ x ⇒ₗ ℓ3, σ₃                   ρ₃, σ₃ ⊢ n ⇒ₗ ℓ2, σ₃
  ───────────────────────────── (Var)    ───────────────────────────── (Var)
  ρ₃, σ₃ ⊢ x ⇒ 40, σ₃                    ρ₃, σ₃ ⊢ n ⇒ 2, σ₃
  ────────────────────────────────────────────────────────────────────────── (Binary)
  ρ₃, σ₃ ⊢ x + n ⇒ 42, σ₃
  ─────────────────────────────────────────────────────────────────────────────────── (Return)
  ρ₃, σ₃ ⊢ return x + n; ⇒ ret 42, ρ₃, σ₃

𝒟₃
                    𝒟₁                      ──────────────────── (Lit)                      𝒟₂
  ρ₂, σ₂ ⊢ f ⇒ closure(int x)[n ↦ 2], σ₂    ρ₂, σ₂ ⊢ 40 ⇒ 40, σ₂          ρ₃, σ₃ ⊢ return x + n; ⇒ ret 42, ρ₃, σ₃
  ─────────────────────────────────────────────────────────────────────────────────────────────────────────────── (CallFn)
  ρ₂, σ₂ ⊢ f(40) ⇒ 42, σ₂
...
𝒟₄
  ...
  ρ₀, σ₀ ⊢ 2 ⇒ 2, σ₀
  ──────────────────────────────────── (Decl)
  ρ₀, σ₀ ⊢ int n = 2; ⇒ normal, ρ₁, σ₁

𝒟₅
  ──────────────────────────────────────────────────────────────────────── (Lambda)
  ρ₁, σ₁ ⊢ [=](int x) -> int { return x + n; } ⇒ closure(int x)[n ↦ 2], σ₁
  ───────────────────────────────────────────────────────────────────────────────── (Decl)
  ρ₁, σ₁ ⊢ c₀ ⇒ normal, ρ₂, σ₂
