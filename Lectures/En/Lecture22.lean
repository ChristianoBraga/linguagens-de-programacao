...
The stack of {secref}[lecture-17] holds integers. A stack of pointers to `Point` would be the same program with `int` replaced by `Point*` throughout, and writing it again is copying. The repetition is not only tedious, it is a maintenance problem, because a correction to one copy has to be made in the others.
...
The declaration mentions `T` where the class of integers mentioned `int`, in the type of the vector, in the parameter of `push` and in the result of `pop`. The uses in `main` write `Stack<int>` and `Stack<Point*>`, and each of them is a type like any other.
...
{exercise "exr-template-nested"}[] Write a template `Lista<T>` whose body mentions `Node<T>`, itself a template, and check with the class table that the expansion reaches a fixed point. Say how many classes the table holds for `Lista<int>`.
...
{exercise "exr-template-erasure"}[] Explain what the store of a program would look like if Core C++ erased the parameter as Java does, and which check the type checker would have to add at each use of `pop`.

{exercise "exr-template-cost"}[] Give two programs that use `Stack<int>`, one with the template and one with the handwritten class of {secref}[lecture-17], and compare their derivations. Say what in the derivation shows that instantiation costs nothing at run time.
